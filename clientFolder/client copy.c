...
int encrypt_symmetric_key(const char *symmetric_key, const char *public_key_file, char **encrypted_key) {
    RSA *public_key = NULL;
    ...
    *encrypted_key = malloc(RSA_size(public_key));
    ...
    int result = RSA_public_encrypt(strlen(symmetric_key), (const unsigned char *)symmetric_key,
                                    (unsigned char *)*encrypted_key, public_key, RSA_PKCS1_PADDING);
...
int main() {
    ...
    if (encrypt_symmetric_key(symmetric_key, "public_key.pem", &encrypted_key) == 0) {
        printf("Symmetric key encrypted successfully.\n");
        printf("Encrypted Key: ");
        for (int i = 0; i < strlen(encrypted_key); i++) {
            printf("%02x", (unsigned char)encrypted_key[i]);
        }
        printf("\n");
